...
struct lemon {
  ...
  struct rule *rule;       /* List of all rules */ int nstate;              
  /* Number of states */ int nrule;               /* Number of rules */ 
  int nsymbol;             /* Number of terminal and nonterminal 
  symbols */ int nterminal;           /* Number of terminal symbols */ 
  struct symbol **symbols; /* Sorted array of pointers to symbols */ 
  int errorcnt;            /* Number of errors */ struct symbol 
  *errsym;   /* The error symbol */ struct symbol *wildcard; /* Token 
  that matches anything */ char *name;              /* Name of the 
  generated parser */ char *arg;               /* Declaration of the 
  3th argument to parser */ char *tokentype;         /* Type of 
  terminal symbols in the parser stack */ char *vartype;           /* 
  The default type of non-terminal symbols */ char *start;             
  /* Name of the start symbol for the grammar */ char *stacksize;         
  /* Size of the parser stack */ char *include;           /* Code to 
  put at the start of the C file */ int  includeln;          /* Line 
  number for start of include code */ char *error;             /* Code 
  to execute when an error is seen */ int  errorln;            /* Line 
  number for start of error code */ char *overflow;          /* Code to 
  execute on a stack overflow */ int  overflowln;         /* Line 
  number for start of overflow code */ char *failure;           /* Code 
  to execute on parser failure */ int  failureln;          /* Line 
  number for start of failure code */ char *accept;            /* Code 
  to execute when the parser excepts */ int  acceptln;           /* 
  Line number for the start of accept code */ char *extracode;         
  /* Code appended to the generated file */ int  extracodeln;        /* 
  Line number for the start of the extra code */ char *includeclass;      
  /* Code insert in the class generated file */ int  includeclassln;     
  /* Line number for the start of the include code */ char *tokendest;         
  /* Code to execute to destroy token data */ int  tokendestln;        
  /* Line number for token destroyer code */ char *vardest;           
  /* Code for the default non-terminal destructor */ int  vardestln;          
  /* Line number for default non-term destructor code*/ char *filename;          
  /* Name of the input file */ char *outname;           /* Name of the 
  current output file */ char *tokenprefix;       /* A prefix added to 
  token names in the .h file */ int nconflict;           /* Number of 
  parsing conflicts */ int tablesize;           /* Size of the parse 
  tables */ int basisflag;           /* Print only basis configurations 
  */ int has_fallback;        /* True if any %fallback is seen in the 
  grammer */ char *argv0;             /* Name of the program */
};
    ...
    case WAITING_FOR_DECL_KEYWORD:
      if( isalpha(x[0]) ){
        ...
        if( strcmp(x,"name")==0 ){
          ...
          psp->declargslot = &(psp->gp->includeclass);
          psp->decllnslot = &psp->gp->includeclassln;
...
/* Append any addition code the parser class */
  tplt_print(out,lemp,lemp->includeclass,lemp->includeclassln,&lineno);
  tplt_xfer(lemp->name,in,out,&lineno);
...
"Illegal stack size: [%s].  The stack size should be an integer constant.",
  ...
  */
  ...
  if (target_lang == LANG_C) {
    ...
    fprintf(out,"public static array $yy_action = [\n"); lineno++;
  }
  ...
  if (target_lang == LANG_C) {
    ...
    fprintf(out,"public static array $yy_lookahead = [\n"); lineno++;
  }
  for(i=j=0; i<n; i++){
  ...
  if (target_lang == LANG_C) {
    ...
    fprintf(out, "public static array $yy_shift_ofst = [\n"); lineno++;
  }
  for(i=j=0; i<n; i++){
    int ofst;
    stp = lemp->sorted[i];
  ...
  if (target_lang == LANG_C) {
    ...
    fprintf(out, "public static array $yy_reduce_ofst = [\n"); lineno++;
  }
  for(i=j=0; i<n; i++){
    int ofst;
    stp = lemp->sorted[i];
  ...
  if (target_lang == LANG_C) {
    ...
    fprintf(out, "public static array $yy_default = [\n"); lineno++;
  }
  n = lemp->nstate;
  for(i=j=0; i<n; i++){
    stp = lemp->sorted[i];
